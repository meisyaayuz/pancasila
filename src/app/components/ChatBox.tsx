...
export function ChatBox({ report, userId, userName, userRole, onMessagesUpdate }: ChatBoxProps) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-[600px]">
      {/* Header */}
      <div className="bg-[#00158b] p-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center">
            <MessageCircle className="w-5 h-5 text-blue-600" />
          </div>
          <div className="flex-1 text-white">
            <h3 className="font-semibold text-lg">Chat dengan {userRole === 'student' ? 'Guru BK' : `Siswa ${report.studentInitial}`}</h3>
      ...
      >
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-gray-600 font-medium mb-2">Belum ada percakapan</p>
            <p className="text-sm text-gray-500 max-w-md">
              {userRole === 'counselor' 
                ? 'Mulai percakapan dengan mengirim pesan kepada siswa. Chat ini bersifat pribadi dan aman.'
                : 'Guru BK siap membantu Anda. Jangan ragu untuk mengobrol jika ada yang ingin dibicarakan.'}
            </p>
          </div>
        ) : (
          <AnimatePresence>
              ...
              return (
                ...
                >
                  ...
                  <div className={`max-w-[70%] ${isOwnMessage ? 'items-end' : 'items-start'} flex flex-col`}>
                    {showAvatar && (
                      <p className={`text-xs text-gray-500 mb-1 px-2 ${isOwnMessage ? 'text-right' : 'text-left'}`}>
        ...
        <div className="flex items-end gap-3">
          <div className="flex-1">
            <textarea
              ...
              className="w-full px-4 py-3 rounded-xl border-2 border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all resize-none"
              rows={2}
            />
            <p className="text-xs text-gray-500 mt-1 ml-1">
              💡 Tips: Tekan Shift+Enter untuk baris baru
            </p>
          </div>
          <button
            type="submit"
            disabled={!message.trim() || isSending}
            className="px-6 py-4 bg-[#00158b] text-white rounded-xl font-medium hover:bg-[#00158b]/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm flex items-center gap-2"
          >
